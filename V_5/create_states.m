...
states = cell(y_state_limit, x_state_limit);

temp_value = [ ; ];

for n = 1:length(series) %Time series cycle
data_first = series{1, n}(1, :); %First angle values
data_second = series{1, n}(2, :); %Second angle values
...
%                   states{j, i} = [states{j, i} temp_value];
            end
        end
